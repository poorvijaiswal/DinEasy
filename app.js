...
app.post('/resto_owner', (req, res) => {
  ...
  con.query(checkid,function(err,result){
      ...
      if(result.length===0){
          ...
          var rid = "SELECT * FROM restaurant WHERE phoneno ='" + rphoneno + "'";
          var q = "SELECT * FROM staff WHERE email='" + email + "'";  // Define q before using it
          ...
          con.query(rid, function (err, result) {
              ...
              con.query(sql, function (err, re) {
                  ...
                  con.query(q, (err, resu) => {
                      ...
                      // Move the cookie setting inside the callback to ensure it's only called after the query has completed
                      res.cookie("user", accessToken, { expire: 2592000000 + Date.now() });
          
                      // Move the sendFile and other response-sending operations here
                      res.sendFile(__dirname + "/dashboard.html");
                      console.log("created");
                  });
              });
...
app.get('/itemdel',authenticateToken,(req, res) => {
  ...
  var sql='DELETE FROM items WHERE rid = "'+req.user.rid+'"and name="'+name+'"and category="'+category+'"';

  con.query(sql,function(err,result){
    if(err)throw err;
    res.render("home3.ejs",{item_data:result,data1:category});
    console.log("item is deleted");

  })
});
  app.get('/itemcat',authenticateToken,(req, res) => {
    ...
    var sql='DELETE FROM cat WHERE rid = "'+req.user.rid+'"and category="'+category+'"';
    console.log(sql);
    con.query(sql,function(err,result){
      if(err)throw err;
    ...
    var sql='DELETE FROM items WHERE rid = "'+req.user.rid+'"and category="'+category+'"';
    console.log(sql);
    con.query(sql,function(err,result){
      if(err)throw err;
    ...
    var sql1="SELECT*FROM cat where rid="+req.user.rid+"" ;
    con.query(sql1,function(err,result){
      if(err)throw err;
      res.render("home2.ejs",{data:result});
    })
//   con.query('DELETE FROM items WHERE rid = "'+req.user.rid+'"and name="'+tno+'"and category="'+category, (err, rows) => {

//     if(!err) {
//       res.render("home3.ejs",{item_data:rows,data1:category});
//     } 
//   
// });
});
